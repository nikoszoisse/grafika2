...
double Object::getDir_z(){
	return dir_z;
}

double Object::getDir_y(){
	return dir_y;
}

double Object::getDir_x(){
	return dir_x;
}
...
bool Object::isOutOfBounds(){
	...
	if(abs(this->x_point>grid_size)||this->y_point<grid_floor||
			fabs(this->z_point)>grid_size){
		return true;
	}

	if(fabs(this->x_target)>grid_size||y_target<grid_floor||
			fabs(this->z_target)>grid_size){
		return true;
	}
