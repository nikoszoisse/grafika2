...
using namespace std;
	...
	void checkIfHeClimbs(){
			...
			if((col_cube=hasCollusion(player,"z"))){
				...
				if(!(upside_cube=hasCollusion(col_cube,"y"))){
					player->setPosition(player_x,player_y+1,player_z+1);
	...
	void createCube(){
		...
		if((standing_cube= hasCollusion(player,"y"))){
			if(standing_cube->apothema==0){
				player->setDirection(new double[3]{dir_x,0.0,dir_z});
				return;
			}
			else{
	...
	void handleCameraView(){
		if(v_Key_pressed_times%5 == 0){
			...
			// Set the camera
			double x_offset = 3*pl_dir[0]*(1-fabs((double)pl_dir[2]));
			double y_offset = 3*pl_dir[2]*(1-fabs((double)pl_dir[0]));
			/*if(camera_changed){
				lx = pl_dir[0];
				lz = pl_dir[2];
			...
			z_eye = player->getZPos()-3;/*-y_offset+3*pl_dir[0]*/
	...
	void pressKey(unsigned char key, int xx, int yy) {
		...
		cout << "key: "<< key <<endl;
	       switch (key) {
	             case 'w':
				 	 player->moveForward();checkIfHeClimbs();break;
